...
ini_set('memory_limit', '10G');  //memory for reads files
	...
	private function readFasta (){
		...
		while (!feof($file )){
			...
			if ($buffer[0]==">"){
				...
				preg_match("/length=(.[^\s]+)/", $buffer, $matches);
				$length	= ( isset($matches[1])?$matches[1]:null);
				preg_match("/xy=(.[^\s]+)/", $buffer, $matches);
				$xy	= ( isset($matches[1])?$matches[1]:null);
				preg_match("/region=(.[^\s]+)/", $buffer, $matches);
				$region	= ( isset($matches[1])?$matches[1]:null);
				preg_match("/run=(.[^\s]+)/", $buffer, $matches);
				$run	= ( isset($matches[1])?$matches[1]:null);
